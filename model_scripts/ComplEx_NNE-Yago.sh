...
#SBATCH --output=/blue/daisyw/ma.haodi/ComplEx-Inject/logs/Yago/ComplEx_NNE-Yago-type_4-all_6.out
#SBATCH --error=/blue/daisyw/ma.haodi/ComplEx-Inject/logs/Yago/ComplEx_NNE-Yago-type_4-all.err
...
T1=`date +%Y%m%d-%H%M%S`
echo ${T1}
singularity exec --nv --bind /blue/daisyw/ma.haodi/ComplEx-Inject/:/home/ComplEx-Inject /blue/daisyw/ma.haodi/kbc_models_version1.8.sif python -u /home/ComplEx-Inject/kbc/learn_logicNN.py --dataset YAGO3-10 --model InjEx --rank 50 --optimizer Adagrad --learning_rate 5e-1 --batch_size 1000 --regularizer N3 --reg 1e-3 --max_epochs 100 --valid 20 --rule_type 4 --mu_1 0.01 --mu_2 0.01
T2=`date +%Y%m%d-%H%M%S`
