...
class Dataset(object):
        ...
        with open(str(self.root / 'rel_id')) as f:
            while True:
                ...
                if line:
                    ...
                    break
        # self.n_predicates *= 2 # reciprocal setting
    ...
    def get_train(self):
        ...
        copy[:, 0] = copy[:, 2]
        copy[:, 2] = tmp
        # copy[:, 1] += self.n_predicates // 2  # reciprocal setting, has been multiplied by two.
        # return np.vstack((self.data['train'], copy)) # reciprocal setting
        return self.data['train'] # non-reciprocal setting
    ...
    def eval(
        ...
        for m in missing:
            ...
            if m == 'lhs':
                ...
                q[:, 0] = q[:, 2]
                q[:, 2] = tmp
                # q[:, 1] += self.n_predicates // 2 # reciprocal setting, removed for non-reciprocal
    ...
    def rank_result(
        ...
        for m in missing:
            ...
            if m == 'lhs':
                ...
                q[:, 0] = q[:, 2]
                q[:, 2] = tmp
                # q[:, 1] += self.n_predicates // 2 # reciprocal setting
            ranks = model.get_ranking(q, self.to_skip[m], batch_size=500)
